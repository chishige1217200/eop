...
void new_profile(struct profile *profile_p, char *line)
{
  ...
  //printf("input:\"%s\"\n", line);                        /*入力文字列をそのまま表示（デバッグ用）*/
  c = split(line, ret, sep, max);                          /*ID，名前などの情報を分割する*/
  if(c <= 2)                                               /*備考以外の入力がない場合*/
    {
      printf("情報はID，名前，誕生日，住所，備考の順で入力される必要があり，備考以外は必須項目です．\n処理を中止しました．\n\n");
  ...
  if(birth_c != 2)                                         /*誕生日の年，月，日を正常に分割できない場合*/
    {
      printf("誕生日は\"年-月-日\"の形で入力される必要があります．\n処理を中止しました．\n\n"); /*年，月，日に分割できない場合，処理を停止*/
      profile_data_nitems--;                               /*処理中止により，構造体に情報を書き込まないため*/
      return;
    }
  ...
  strncpy(profile_p->name, ret[1], 70);                    /*名前の書き込み*/
  ...
  strncpy(profile_p->address, ret[3], 70);                 /*住所の書き込み*/
  if(ret[4] != NULL)
    strncpy(profile_p->biko, ret[4], 70);                  /*備考があるときのみ，備考の書き込み*/

  printf("id:\"%d\"\n", profile_p->id);
  printf("name:\"%s\"\n", profile_p->name);
  printf("birthday:\"%d-%d-%d\"\n", (profile_p->birthday).y, (profile_p->birthday).m, (profile_p->birthday).d);
  printf("address:\"%s\"\n",profile_p->address);
  printf("biko:\"%s\"\n\n",profile_p->biko);
